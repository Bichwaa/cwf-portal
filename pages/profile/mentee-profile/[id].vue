<template>
  <div class="min-h-screen bg-gray-100 p-8">
    ...
    <!-- Page Header -->
    <div class="mb-8">
      <h1 class="text-2xl text-left font-bold">{{ mentor.user.firstName + ' ' + mentor.user.lastName }}</h1>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <h3 class="text-lg text-start font-semibold text-gray-800 mb-4">Accept {{ mentor.user.firstName  }}'s request</h3>
          <hr class="my-4"/>
          <div class="space-y-4">
            <button 
            ...
            >
              <span v-if="isRequesting">accepting...</span>
              <span v-else-if="isRequestSuccessful">Request Accepted</span>
              <span v-else>Accept Request</span>
            </button>
          </div>
        </div>
    ...
    <MenteeAcceptedModal v-if="showSuccessModal && mentor" :mentee="mentor.user" @close="closeSuccessModal" />
  </div>
</template>

<script lang="ts" setup>
import MentorPersonalInfoCard from '@/components/mentors/MentorPersonalInfoCard.vue'
import MenteeAcceptedModal from '~/components/MenteeAcceptedModal.vue'
...
onMounted(async () => {
  ...
  try {
    ...
    // Fetch current user
    const userResponse = await get<any>('/auth/me', {
      headers: { authorization: `Bearer ${token}` }
    })
    currentUser.value = {
